...
for direction_id, steps in INSTRUCTIONS:
    ...
    for _ in range(int(steps)):
        knots[0] = add_coords(knots[0], direction)

        for knot_index in range(1, ROPE_LENGTH):
            if axis_distance(knots[knot_index - 1], knots[knot_index]) <= 1:
                continue

            possible_moves = []

            for possible_move in MOVES:
                new_position = add_coords(knots[knot_index], possible_move)
                possible_moves.append(
                    (distance(new_position, knots[knot_index - 1]), possible_move)
                )

            possible_moves.sort()
            best_move = possible_moves[0][1]

            knots[knot_index] = add_coords(knots[knot_index], best_move)
        visited.add(knots[-1])
